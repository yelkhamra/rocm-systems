...
- **Dynamic Binary Translation** - cross-ISA instruction legalization tables
  and encoding translator data tables are auto-generated by `amdisa` from
  the MR ISA XML specs, enabling translation between any supported ISA pair.
  ...
  rocjitsu/             Main project library
    ...
    src/rocjitsu/       Private headers and sources
      ...
      isa/              ISA framework (decoder, instruction, operand)
        arch/amdgpu/    AMDGPU ISA (all 9 targets, mostly autogenerated)
        arch/risc_v/    RISC-V ISA (RV32IMAFDC)
        dbt/            Encoding translator engine and coherency remap helpers
...
The cross-ISA legalization tables and encoding translator data tables are also
autogenerated from the MR ISA XML specs. These are used by the dynamic binary
translation (DBT) subsystem.

```bash
# Legalization tables (all supported ISA pairs)
