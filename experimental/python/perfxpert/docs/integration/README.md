...
| Topic | Doc | Audience |
|-------|-----|----------|
| MCP server (`perfxpert-mcp`) — 60 READ_ONLY tools re-exposed to any MCP-compatible client (opencode, Claude Desktop, Cursor, …) over stdio. **8 of the 60 are agent-hierarchy tools** (`perfxpert_agent_root`, `perfxpert_agent_analysis`, `perfxpert_agent_recommendation`, `perfxpert_agent_correctness`, `perfxpert_agent_compute_specialist`, `perfxpert_agent_memory_specialist`, `perfxpert_agent_latency_specialist`, `perfxpert_agent_diff_specialist`) — 1:1 mirrored in `perfxpert.api`. The other 52 are classifier, knowledge, retained-history, and trace-query tools. Also documents the dot→underscore tool-name mangling. | [mcp-server.md](mcp-server.md) | External MCP client authors; integrators that need read-only access to PerfXpert's knowledge and classifiers |
