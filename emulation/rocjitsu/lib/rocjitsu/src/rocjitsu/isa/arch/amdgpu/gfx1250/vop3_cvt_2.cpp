...
void VCvtPkFp8F32Vop3::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    uint32_t lo = (inst_.clamp)
                      ? util::f32_to_fp8_e5m3_rne(s0)
                      : util::f32_to_fp8_e4m3_rne_with_saturation(s0, wf.fp16_overflow_saturates());
    uint32_t hi = (inst_.clamp)
                      ? util::f32_to_fp8_e5m3_rne(s1)
                      : util::f32_to_fp8_e4m3_rne_with_saturation(s1, wf.fp16_overflow_saturates());
...
void VCvtSrFp8F32Vop3::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    uint8_t result =
        (inst_.clamp)
            ? util::f32_to_fp8_e5m3_sr(s0, seed)
