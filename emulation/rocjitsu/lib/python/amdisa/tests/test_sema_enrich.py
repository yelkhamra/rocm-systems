...
class TestRtnAtomicFix:
        ...
        body = SemaNode(
            ...
            children=(
                ...
                SemaNode(
                    ...
                    children=(
                        _cast(
                            SemaNode(SemaNodeKind.ID, id_name='RETURN_DATA'),
                            SemaType.U32,
                        ),
                        SemaNode(SemaNodeKind.ID, id_name='tmp'),
                    ),
        ...
        for c in enriched.body.children:
            if c.kind == SemaNodeKind.ASSIGN:
                ...
                while lhs.kind == SemaNodeKind.CAST and lhs.children:
                    lhs = lhs.children[0]
                assert not (
                    lhs.kind == SemaNodeKind.ID and lhs.id_name == 'RETURN_DATA'
    ...
    def test_rtn_keeps_return_data(self):
        ...
        enriched = enrich_block(block)
        has_return = any(
            c.kind == SemaNodeKind.ASSIGN and c.children[0].kind == SemaNodeKind.CAST
            for c in enriched.body.children
        )
        assert has_return
...
class TestVop3Modifiers:
    ...
    def test_no_fields_no_modifiers(self):
        ...
        all_calls = [n for n in enriched.body.walk() if n.kind == SemaNodeKind.CALL]
        assert len(all_calls) == 0

    def test_neg_abs_wraps_sources(self):
        block = self._make_vop3_add()
        enriched = enrich_block(block, enc_field_names=frozenset({'neg', 'abs'}))
        src_mod_calls = [
            n
            for n in enriched.body.walk()
            if n.kind == SemaNodeKind.CALL and n.call_name == 'apply_src_mod'
        ]
    ...
    def test_clamp_omod_wraps_dest(self):
        ...
        clamp_calls = [
            n
            for n in enriched.body.walk()
            if n.kind == SemaNodeKind.CALL and n.call_name == 'apply_clamp'
        ]
        omod_calls = [
            n
            for n in enriched.body.walk()
            if n.kind == SemaNodeKind.CALL and n.call_name == 'apply_omod'
        ]
...
class TestBuildSemaBlock:
    ...
    def test_with_enc_fields(self):
        ...
        assert result is not None
        call_names = [
            n.call_name for n in result.body.walk() if n.kind == SemaNodeKind.CALL
        ]
        assert 'apply_src_mod' in call_names
        assert 'apply_clamp' in call_names
...
class TestSemaXmlEnrichment:
    ...
    def test_non_rtn_count(self, blocks):
        ...
        for name, b in blocks.items():
            ...
            new_len = (
                len(enriched.body.children)
                if enriched.body.kind == SemaNodeKind.SEQ
                else 0
            )
            if new_len < orig_len:
                count += 1
