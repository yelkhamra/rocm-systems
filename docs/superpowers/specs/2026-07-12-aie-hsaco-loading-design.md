...
| `KERNEL_OBJECT` handle | Device VA of `amd_kernel_code_t` | Opaque handle to agent-owned descriptor |
| Symbol table | Compiler-emitted `foo.kd` symbols | Kernel table in section header; loader creates one `AieKernelSymbol` each |
| Packet → kernel binding | Packet carries VA; CP reads descriptor on-device | Packet carries opaque handle; **runtime** resolves via agent |
| Who reads descriptor | Hardware command processor | Host runtime at submit time (MVP) |
...
### 4. AIE Agent Descriptor Registry

The AIE agent (`amd_aie_agent`) owns the loaded descriptors and their underlying
device buffers.

- On load, the agent registers each `AieKernelDescriptor` and returns an opaque
  handle. The handle value is what `KERNEL_OBJECT` reports and what the packet
  carries.
- At **submit time**, the runtime resolves the packet's `kernel_object` handle
  against the agent's registry to obtain `pdi_dev_addr` / `insts_dev_addr` /
  `insts_size`, then feeds the existing XDNA driver path (which reconfigures the
  hardware context when it observes a new PDI). When `pdi_dev_addr == 0`, PDI
  configuration is skipped.
- Descriptors and device buffers are freed when their owning code object is
  destroyed; each object owns its own resources so unload is independent.

### 5. Packet Change (`hsa_ext_amd_aie.h`)

Required now, to avoid leaking the internal descriptor structure to users.

- Replace the raw `insts_addr_low` / `insts_addr_high` / `insts_size` /
  `pdi_addr` fields with a single opaque **`kernel_object`** handle field (a
  `uint64_t`, matching the GPU `hsa_kernel_dispatch_packet_t::kernel_object`).
- `kernarg_address` / `num_kernargs` semantics are unchanged.
- The test and any packet-construction helper set `kernel_object` from
  `HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT`; the runtime resolves PDI/insts.
...
- `runtime/hsa-runtime/core/runtime/amd_aie_agent.cpp`,
  `core/inc/amd_aie_agent.h` — descriptor registry + handle resolution.
...
- **Descriptor handle namespace:** handles must be unambiguous across multiple
  loaded objects on the same agent and invalidated on unload.
