...
* **Memory peak dropdown:** Use the *Memory peak* selector at the top of the page to view kernel points against a single roof at a time or against *All peaks* at once. Each kernel is drawn in its own color, and the marker shape indicates the memory peak.
* **Kernel filtering:** The *Kernels* panel to the right lists every kernel. Click a kernel row -- or click one of its dots in the plot -- to isolate just that kernel; click it again to show all. Ctrl-click (Cmd-click on macOS) to add or remove kernels from the selection, and use *Show all kernels* to reset. Every selected kernel shows a per-peak details table (peak, arithmetic intensity, performance, and whether each point is memory- or compute-bound), stacked together so you can compare multiple kernels at once. Long kernel names are truncated in the table headings and hover tooltip; hover over a name to see it in full.
