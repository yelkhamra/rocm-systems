...
class PCSamplingProfile:
    ...
    def __init__(
        ...
        self._native_tool_path = native_tool_path
    ...
    def _cleanup_stale_output(
        ...
        """Remove leftover native artifacts and a stale ROCPROF_OUTPUT_PATH."""
        self._cleanup_native_artifacts()
        self._cleanup_rocprof_output_path(profiler_options)

    def _cleanup_native_artifacts(self) -> None:
        """Remove native PC sampling artifacts left in the workload dir."""
    ...
    def _cleanup_rocprof_output_path(
        self,
        profiler_options: ProfilerOptions,
    ) -> None:
        """Remove a stale ROCPROF_OUTPUT_PATH for PC-sampling-only sdk runs."""
        if not (
    ...
    def _launch(
        ...
        if self._profiler == "rocprofiler-sdk":
            if self._native_tool_path and not is_live_attach(profiler_options):
                self._launch_native(
    ...
    def _build_env(
        ...
        log_label: str,
    ) -> tuple[Optional[Union[str, list[str]]], dict]:
    ...
    def _launch_native(
        ...
        """Launch the native PC sampling tool via LD_PRELOAD and env vars."""
        options = profiler_options.copy()
        original_ld_preload = os.environ.get("LD_PRELOAD")
        options["LD_PRELOAD"] = ":".join(
            p for p in [original_ld_preload, self._native_tool_path] if p
        )
        options.update({
            "ROCPROF_COUNTER_COLLECTION": "0",
            "ROCPROF_OUTPUT_PATH": self._workload_dir,
        ...
        app_cmd, new_env = self._build_env(options, "pc sampling native env vars")
        self._run_app(app_cmd, new_env)
