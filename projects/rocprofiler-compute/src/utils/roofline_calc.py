...
# Datatype -> ops support shared by every CDNA arch that benchmarks matrix ops.
_CDNA_MATRIX_DATATYPES: dict[str, OpsSupport] = {
...
# gfx115x turns off WMMA benchmarking for every precision (the underlying tools
# do not surface the matrix counters), so every datatype is VALU-only.
_RDNA_VALU_DATATYPES: dict[str, OpsSupport] = {
    dtype: OpsSupport.VALU
    for dtype in ("FP16", "BF16", "FP32", "FP64", "I8", "I32", "I64")
}
...
SUPPORTED_DATATYPES: dict[str, dict[str, OpsSupport]] = {
    # gfx90a is the CDNA matrix set without FP8.  Unsupported: F4, F6, F8
    "gfx90a": {k: v for k, v in _CDNA_MATRIX_DATATYPES.items() if k != "FP8"},
    "gfx940": _CDNA_MATRIX_DATATYPES,  # Unsupported: F4, F6
    "gfx941": _CDNA_MATRIX_DATATYPES,  # Unsupported: F4, F6
    "gfx942": _CDNA_MATRIX_DATATYPES,  # Unsupported: F4, F6
    # gfx950 adds the low-precision matrix formats on top of the CDNA set.
    "gfx950": {
        "FP4": OpsSupport.MATRIX,
        "FP6": OpsSupport.MATRIX,
        **_CDNA_MATRIX_DATATYPES,
    },
    "gfx1150": _RDNA_VALU_DATATYPES,  # Unsupported: F4, F6, F8
    "gfx1151": _RDNA_VALU_DATATYPES,  # Unsupported: F4, F6, F8
    "gfx1152": _RDNA_VALU_DATATYPES,  # Unsupported: F4, F6, F8
}
