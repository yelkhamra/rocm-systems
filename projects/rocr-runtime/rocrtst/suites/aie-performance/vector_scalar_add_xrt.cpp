...
// Helper to load instruction binary into a vector.
...
static void VectorScalarAddXRTAsync(benchmark::State& state) {
  // Load instructions
  std::vector<uint32_t> instr_v = load_instr_binary(g_insts_path);
  ...
  for (auto _ : state) {
    for (std::int32_t i = 0; i < num_dispatches; ++i) {
      bo_ins[i].sync(XCL_BO_SYNC_BO_TO_DEVICE);
      const unsigned int opcode = 3;
      runs[i] = kernel(opcode, bo_instr, instr_v.size(), bo_ins[i], bo_outs[i]);
    }
...
BENCHMARK(VectorScalarAddXRTAsync)->Unit(benchmark::kMicrosecond)->RangeMultiplier(2)->Range(1, 32);
