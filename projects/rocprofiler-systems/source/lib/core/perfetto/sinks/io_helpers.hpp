...
write_proto_to(const std::string& filename, const char* data, std::size_t size,
               status_emission emit_status = status_emission::verbose)
{
    ...
    if(!parent.empty())
    {
        ...
        if(ec)
        {
            ...
            if(emit_status == status_emission::verbose) emit_open_error_line(filename);
            return false;
        }
    }
    ...
    if(emit_status == status_emission::verbose) emit_size_line(filename, size);
