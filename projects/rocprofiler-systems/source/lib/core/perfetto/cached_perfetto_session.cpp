...
single_file_sink
make_merged_append_sink(std::size_t source_count)
{
    ...
    auto       sink        = single_file_sink{ merged_path };
...
make_sink(pid_t root_pid, bool combine_traces, std::size_t source_count)
{
    if(combine_traces)
    {
        return std::make_unique<trace_sink>(make_merged_append_sink(source_count));
    }

    return std::make_unique<trace_sink>(per_pid_file_sink{ root_pid });
}
}  // namespace

cached_perfetto_session::cached_perfetto_session(pid_t root_pid, bool combine_traces,
...
, m_sink{ make_sink(root_pid, combine_traces, source_pids.size()) }
