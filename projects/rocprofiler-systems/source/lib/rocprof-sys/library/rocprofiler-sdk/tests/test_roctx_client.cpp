...
// session::shutdown() clears subscribers + votes only; trigger state
// (e.g. the range filter) persists, since triggers are owned by the client.
// With no votes recorded, the resolved state defaults back to active.
