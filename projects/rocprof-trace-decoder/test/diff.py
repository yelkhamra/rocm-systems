...
def diff_csv_files(file1, file2):
    ...
    filtered1 = [data1[0]] + [row for row in data1[1:] if not is_ignorable_row(header, row)] if data1 else []
    filtered2 = [data2[0]] + [row for row in data2[1:] if not is_ignorable_row(header, row)] if data2 else []
