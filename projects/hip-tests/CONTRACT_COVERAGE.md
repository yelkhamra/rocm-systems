...
- Declared HIP runtime APIs directly exercised by contract tests: 440
- Approximate declared API-name coverage: 89.1%
...
2. Texture and surface APIs beyond current basics: runtime and driver-style texture object create/destroy with resource, texture, and resource-view descriptor round-trips, image-gated device texture-width queries, surface object create/destroy, and channel-descriptor queries are now covered; mipmapped arrays are now covered too (`hipMallocMipmappedArray`/`hipFreeMipmappedArray`, driver-style `hipMipmappedArrayCreate`/`hipMipmappedArrayDestroy`, level retrieval via `hipGetMipmappedArrayLevel` and `hipMipmappedArrayGetLevel` with an out-of-range rejection, and `hipMipmappedArrayGetMemoryRequirements`), image-gated so they skip where image support is absent (as on the local WSL2 iGPU) and are exercised on datacenter arches. Texture reference APIs and bound/linear texture variants remain.
