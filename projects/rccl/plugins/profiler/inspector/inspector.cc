...
inspectorDumpThread::inspectorDumpThread(const char* _outputRoot, int64_t _sampleIntervalUsecs)
  ...
  // RCCL: init the condvar used for the interruptible dump-thread sleep. Prefer
  // a monotonic clock so the deadline is immune to wall-clock jumps; fall back
  // to CLOCK_REALTIME if the platform won't honor the request.
...
inspectorDumpThread::~inspectorDumpThread() {
  ...
  // RCCL: tear down the interruptible-sleep condvar (see constructor).
...
void inspectorDumpThread::stopThread() {
  ...
  run = false;
  inspectorUnlockRWLock(&guard);
  // RCCL: wake dumpMain() immediately instead of letting it sleep out the
  // remaining sampleIntervalUsecs (fixes ncclCommDestroy teardown hang).
...
void* inspectorDumpThread::dumpMain(void* arg) {
  ...
  while (dumper->run) {
    ...
    // RCCL: interruptible sleep via condvar so stopThread() can wake us up at
    // teardown instead of blocking out the full sampleIntervalUsecs (fixes
    // ncclCommDestroy hang). NCCL uses std::this_thread::sleep_for here; do not
    // sync from NCCL.
    if (dumper->sampleIntervalUsecs > 0) {
      struct timespec deadline;
      clock_gettime(dumper->sleepCondClock, &deadline);
      uint64_t addNsec = (uint64_t)deadline.tv_nsec +
                         ((uint64_t)dumper->sampleIntervalUsecs % 1000000ULL) * 1000ULL;
      deadline.tv_sec += (time_t)((uint64_t)dumper->sampleIntervalUsecs / 1000000ULL) +
                         (time_t)(addNsec / 1000000000ULL);
      deadline.tv_nsec = (long)(addNsec % 1000000000ULL);
...
void inspectorComputeOpBw(struct inspectorCommInfo *commInfo,
  ...
  if (op->isP2p) {
    ...
    case ncclFuncReduceScatter:
      ...
      trafficSize = (double)op->msgSizeBytes * (double)commInfo->nranks;
...
static uint64_t calculateKernelGpuExecTimeUsecs(struct inspectorKernelChInfo *kernelCh) {
  if (kernelCh->startGpuClk != 0 && kernelCh->stopGpuClk != 0) {
    if (kernelCh->stopGpuClk > kernelCh->startGpuClk) {
      ...
      // RCCL: AMD wall_clock64() runs at 100 MHz (10 ns/tick); divide by 100 for us.
