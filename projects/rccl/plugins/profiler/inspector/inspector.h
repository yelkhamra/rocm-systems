...
struct inspectorDumpThread {
  ...
  // RCCL: condvar-based interruptible sleep for the dump thread. stopThread()
  // signals sleepCond after clearing run, so dumpMain() wakes immediately
  // instead of sleeping out sampleIntervalUsecs (fixes teardown hang). NCCL
  // uses std::this_thread::sleep_for and has no upstream counterpart for these.
  pthread_mutex_t sleepMutex;
  pthread_cond_t  sleepCond;
  clockid_t       sleepCondClock{CLOCK_MONOTONIC};
