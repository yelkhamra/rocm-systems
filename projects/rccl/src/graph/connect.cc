...
ncclResult_t ncclTopoPostset(struct ncclComm* comm, int* firstRanks, int* treePatterns,
  ...
  if (comm->sharedRes->owner != comm) {
    ...
    nChannels = comm->nChannels = copyChannels(comm, nChannels,
                                               std::min(std::min(comm->config.maxCTAs, comm->sharedRes->tpNChannels),
                                                        std::max(minNchannels, std::max(nc, comm->config.minCTAs))),
                                               ringPrev, ringNext);
  } else {
    nChannels = comm->nChannels =
    ...
    nChannels = comm->nChannels =
      copyChannels(comm, nChannels,
                   std::min(comm->config.maxCTAs, std::max(minNchannels, std::max(nc, comm->config.minCTAs))), ringPrev,
                   ringNext);
  }
